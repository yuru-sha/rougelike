...
class Player(Entity):
    ...
    def __init__(self, x: int, y: int):
        super().__init__(x=x, y=y, char="@", name="Player")
        self.is_dead = False
    ...
    def move(self, dx: int, dy: int, game_map: GameMap) -> bool:
        ...
        new_y = self.y + dy
        logger.debug(
            f"Attempting to move from ({self.x}, {self.y}) to ({new_x}, {new_y})"
        )
    ...
    def _get_current_room(self, game_map: GameMap) -> Optional[Room]:
        ...
        for room in game_map.rooms:
            if (
                room.x <= self.x < room.x + room.width
                and room.y <= self.y < room.y + room.height
            ):
                return room
        return None
    ...
    def _has_line_of_sight(
        self, game_map: GameMap, target_x: int, target_y: int
    ) -> bool:
